...
$(function () {
    ...
    //(4)将字符串转为真正的数组
    console.log([].concat(_toConsumableArray2("hello"))); //["h", "e", "l", "l", "o"]

    //JavaScript会将32位Unicode字符，识别为2个字符，采用扩展运算符就没有这个问题。
    //凡是涉及到操作32位Unicode字符的函数，都有这个问题。因此，最好都用扩展运算符改写。
    'x\uD83D\uDE80y'.length; //4
    [].concat(_toConsumableArray2('x\uD83D\uDE80y')).length; //3  => [].concat(_toConsumableArray('x\uD83D\uDE80y')).length;
    ...
    var arrLike1 = [].concat(_toConsumableArray2(arraylike)); //[11,22,33]
    ...
    var arrMap = [].concat(_toConsumableArray2(map.keys())); //[1,2,3]   => es5 var arrMap = [].concat(_toConsumableArray2(map.keys()));

    var set = new Set([1, 2, 3]);
    var arrSet = [].concat(_toConsumableArray2(set)); //[1,2,3]  =>es5  var arrSet = [].concat(_toConsumableArray2(set));
